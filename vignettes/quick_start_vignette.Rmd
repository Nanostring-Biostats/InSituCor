---
title: "SPARC quick start vignette"
output: 
...
fig_height: 8 
vignette: >
  %\VignetteIndexEntry{Straightforward SPARC run}
...
#devtools::install_github("https://github.com/Nanostring-Biostats/SPARC")
# setwd() in Rstudio to the SPARC directory
...
The SPARC algorithm identifies modules of "spatially co-expressed" genes - i.e.
...
Once spatially co-expressed modules have been detected, SPARC provides a number 
of tools to help explore and interpret them.

SPARC attempts to weed out "uninteresting" or trivial spatial correlations, for example
...
This vignette demonstrates the use of the SPARC algorithm to find groups of spatially co-expressed genes in [CosMx](https://www.nature.com/articles/s41587-022-01483-z) data. CosMx is a spatial molecular imager for sub-cellular resolution of mRNA transcripts and protein targets. We analyze a subset of a lupus nephritis experiment. 
...
4. Cell type assignments. SPARC uses this information to assess the role of
...
The complete SPARC workflow consists of 4 steps:
...
These steps can be run separately, or they can be run all at once using the sparc() function.
...
res <- SPARC::sparc(
               ...
               neighbors = NULL, xy = xy, k = NULL, radius = 0.1, tissue = annot$fov, 
               # args for module definition:
               min_module_size = 2, max_module_size = 20, min_module_cor = 0.05,
...
### Workflow for exploring SPARC results:

Once you've run SPARC, the we recommend exploring your results as follows:
...
Advanced users wishing for more control can skip the main \code{sparc}
